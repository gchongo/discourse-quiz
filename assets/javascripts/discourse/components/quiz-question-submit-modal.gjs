...
export default class QuizQuestionSubmitModal extends Component {
  ...
  constructor() {
    ...
    this.useNewCategory = categories.length === 0;
  }

  get modalTitle() {
    return i18n("discourse_quiz.question_submission.title");
  }
  ...
  async saveQuestion() {
    ...
    try {
      await ajax("/quiz/question_submissions.json", {
        type: "POST",
        data: { question_submission: this.questionPayload() },
      });
