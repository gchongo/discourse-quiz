...
export default class QuizSubmissionEditModal extends Component {
  ...
  constructor() {
    super(...arguments);
    const submission = this.args.model.submission || {};

    this.categoryName = submission.category_name || "";
    this.questionText = submission.question_text || "";
    this.questionType = submission.question_type || "single_choice";
    this.optionsText = (submission.options || []).join("\n");
    this.correctIndex = submission.correct_index ?? 0;
    this.correctIndices = Array.isArray(submission.correct_indices)
      ? submission.correct_indices
          .map((value) => Number(value))
          .filter((value) => Number.isFinite(value))
      : [];
    this.explanation = submission.explanation || "";
    this.showAuthorName = submission.show_author_name !== false;

    const categories = this.args.model.categories || [];
    if (!this.categoryName && categories.length) {
      this.categoryName = [...categories].sort((a, b) => a.localeCompare(b, "zh-CN"))[0];
    }
    this.useNewCategory = !categories.length;
  }

  get modalTitle() {
    return i18n("discourse_quiz.admin.question_submissions_edit_title");
  }
  ...
  async saveSubmission() {
    ...
    try {
      await ajax(`/admin/quiz/question_submissions/${this.args.model.submission.id}/edit.json`, {
        type: "PUT",
        data: { submission: this.submissionPayload() },
      });
